...
namespace UniverseLib.Runtime.Il2Cpp
{
    internal class Il2CppProvider : RuntimeHelper
    {
        ...
        {
            if (!scene.isLoaded || scene.GetIntHandle() == -1)
            {
                return [];
            }

            int count = GetRootCount(scene.GetIntHandle());
            if (count < 1)
            {
                return [];
            }
            ...
            ICallManager.GetICall<d_GetRootGameObjects>("UnityEngine.SceneManagement.Scene::GetRootGameObjectsInternal")
                .Invoke(scene.GetIntHandle(), list.Pointer);
            return list.ToArray();
        }

        /// <inheritdoc/>
        protected internal override int Internal_GetRootCount(Scene scene) => GetRootCount(scene.GetIntHandle());
